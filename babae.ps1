...
function Get-ClipboardText {
  ...
  try {
    ...
    Write-DiagLog 'CLIP' "Clipboard read failed: $($_.Exception.Message)"
  }
...
function Set-ClipboardText([string]$text) {
  ...
  try {
    ...
    Write-DiagLog 'CLIP' "Clipboard write failed: $($_.Exception.Message)"
  }
}
...
'@ -ErrorAction SilentlyContinue
    ...
    Write-DiagLog 'INPUT' "Windows mouse support init failed: $($_.Exception.Message)"
  }
}
...
function LineCount {
  return [Math]::Max(1, $script:lineIndex.Length)
}
...
function State-Snapshot {
  if ($state.UndoStack.Count -ge $script:undoStackMax) {
    ...
    for ($i = [Math]::Min($script:undoStackTrim, $arr.Count) - 1; $i -ge 0; $i--) {
      $state.UndoStack.Push($arr[$i])
    }
  }
